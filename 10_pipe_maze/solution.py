    ...
    def move(self):
        for _y, _x in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
            ...
            if not 0 <= self._current_x + _x < self._tiles.shape[1]:
                continue
            if (_y == 1 and self._tiles[self._current_y, self._current_x] in ['|', 'F', '7', 'S']
                    and self._tiles[self._current_y + _y, self._current_x] in ['|', 'J', 'L', 'S']):
                self._current_y += _y
                self._last_direction = (_y, 0)
                return
            if (_x == 1 and self._tiles[self._current_y, self._current_x] in ['-', 'F', 'L', 'S']
                    and self._tiles[self._current_y, self._current_x + _x] in ['-', 'J', '7', 'S']):
                self._current_x += _x
                self._last_direction = (0, _x)
                return
            if (_y == -1 and self._tiles[self._current_y, self._current_x] in ['|', 'L', 'J', 'S']
                    and self._tiles[self._current_y + _y, self._current_x] in ['|', '7', 'F', 'S']):
                self._current_y += _y
                self._last_direction = (_y, 0)
                return
            if (_x == -1 and self._tiles[self._current_y, self._current_x] in ['-', 'J', '7', 'S']
